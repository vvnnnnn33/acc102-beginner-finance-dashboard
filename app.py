...
def get_data_paths(ticker):
    market_path = os.path.join("data", f"{ticker}_market.csv")
    fund_path = os.path.join("data", f"{ticker}_fund.csv")
    return market_path, fund_path


def load_market_data(ticker, start_date, end_date):
    market_path, _ = get_data_paths(ticker)
    ...
    df = df.dropna(subset=["date", "prc"]).copy()
...
def load_fundamental_data(ticker, start_year, end_year):
    _, fund_path = get_data_paths(ticker)
    ...
    numeric_cols = ["fyear", "sale", "ni", "act", "lct", "invt", "profit_margin", "profit_margin_pct", "quick_ratio", "current_ratio"]
    ...
    # 如果导出的 csv 里还没有这些列，就在 app 里补算
    if "profit_margin" not in df.columns and {"ni", "sale"}.issubset(df.columns):
        df["profit_margin"] = df["ni"] / df["sale"]

    if "profit_margin_pct" not in df.columns and "profit_margin" in df.columns:
        df["profit_margin_pct"] = df["profit_margin"] * 100

    if "quick_ratio" not in df.columns and {"act", "invt", "lct"}.issubset(df.columns):
        df["quick_ratio"] = (df["act"] - df["invt"]) / df["lct"]

    if "current_ratio" not in df.columns and {"act", "lct"}.issubset(df.columns):
...
def build_fundamental_summary(fund_df):
    if fund_df.empty:
        return {
            ...
            "latest_quick_ratio": None,
            "latest_current_ratio": None,
            "profit_margin_comment": "not enough financial data",
            "quick_ratio_comment": "not enough financial data",
            "current_ratio_comment": "not enough financial data",
        }

    latest_profit_margin = float(fund_df["profit_margin_pct"].iloc[-1]) if "profit_margin_pct" in fund_df.columns else None
    latest_quick_ratio = float(fund_df["quick_ratio"].iloc[-1]) if "quick_ratio" in fund_df.columns else None
    latest_current_ratio = float(fund_df["current_ratio"].iloc[-1]) if "current_ratio" in fund_df.columns else None

    return {
        "latest_revenue": float(fund_df["sale"].iloc[-1]) if "sale" in fund_df.columns else None,
        "latest_net_profit": float(fund_df["ni"].iloc[-1]) if "ni" in fund_df.columns else None,
        "latest_profit_margin": latest_profit_margin,
        "latest_quick_ratio": latest_quick_ratio,
        "latest_current_ratio": latest_current_ratio,
        "profit_margin_comment": get_profit_margin_comment(latest_profit_margin) if latest_profit_margin is not None else "not enough financial data",
        "quick_ratio_comment": get_quick_ratio_comment(latest_quick_ratio) if latest_quick_ratio is not None else "not enough financial data",
        "current_ratio_comment": get_current_ratio_comment(latest_current_ratio) if latest_current_ratio is not None else "not enough financial data",
    }
...
def display_fundamental_summary(company_name, fund_summary):
    ...
    if fund_summary["latest_revenue"] is not None:
        st.write(f"Latest revenue: {fund_summary['latest_revenue']:,.2f} USD mn")
    else:
        st.write("Latest revenue: not enough financial data")

    if fund_summary["latest_net_profit"] is not None:
        st.write(f"Latest net profit: {fund_summary['latest_net_profit']:,.2f} USD mn")
    else:
        st.write("Latest net profit: not enough financial data")

    if fund_summary["latest_profit_margin"] is not None:
        st.write(f"Latest profit margin: {fund_summary['latest_profit_margin']:.2f}%")
    else:
        st.write("Latest profit margin: not enough financial data")

    if fund_summary["latest_quick_ratio"] is not None:
        st.write(f"Latest quick ratio: {fund_summary['latest_quick_ratio']:.2f}x")
    else:
        st.write("Latest quick ratio: not enough financial data")

    if fund_summary["latest_current_ratio"] is not None:
        st.write(f"Latest current ratio: {fund_summary['latest_current_ratio']:.2f}x")
    else:
        st.write("Latest current ratio: not enough financial data")
...
if run_analysis:
    ...
    market_data_a = load_market_data(ticker_a, start_date, end_date)
    fund_data_a = load_fundamental_data(ticker_a, start_date.year, end_date.year)

    if fund_data_a.empty:
        st.error(f"No financial data file was found for {company_a}. Please check data/{ticker_a}_fund.csv")
        st.stop()
    ...
    if market_data_a.empty or market_summary_a["total_return_pct"] is None:
        st.warning(f"{company_a} does not have enough market data in the selected date range to produce a full market analysis.")
    else:
    ...
    if has_company_b:
        market_data_b = load_market_data(ticker_b, start_date, end_date)
        fund_data_b = load_fundamental_data(ticker_b, start_date.year, end_date.year)

        if fund_data_b.empty:
            st.error(f"No financial data file was found for {company_b}. Please check data/{ticker_b}_fund.csv")
            st.stop()
    ...
    concept_explanations = {
        ...
        "Ticker": {
            ...
            "importance": "It is needed to retrieve market and financial statement data from the saved dataset.",
            "formula": "Not a calculated indicator.",
